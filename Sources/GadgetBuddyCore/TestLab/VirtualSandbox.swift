...
public struct VirtualSandbox: Sendable {
    private let interpreter: CommandInterpreter
    private let challenger: Challenger

    public init(interpreter: CommandInterpreter = CommandInterpreter(), challenger: Challenger = Challenger()) {
    ...
    public func run(_ scenario: SandboxScenario) -> SandboxReport {
        ...
        let command = interpreter.interpret(scenario.input)
